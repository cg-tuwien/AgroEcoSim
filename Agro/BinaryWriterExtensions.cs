...
public static class BinaryWriterExtensions
{
	...
	[M(AI)] public static void WriteQ32(this BinaryWriter writer, Quaternion q)
	{
		writer.Write(q.X); writer.Write(q.Y); writer.Write(q.Z); writer.Write(q.W);
	}

	[M(AI)]	public static void WriteV32ZY(this BinaryWriter writer, Vector3 xyz, float offset, float scale)
	{
		var v = xyz * scale + new Vector3(offset);
		writer.Write(v.X);
		writer.Write(v.Z);
		writer.Write(v.Y);
	}
