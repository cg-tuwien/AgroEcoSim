...
public partial class SoilFormation : Formation3iTransformed<SoilAgent>
{
	...
	public float GetWaterCapacity(AgroWorld world, int index) => index >= 0 && index < Agents.Length
		? (ReadTMP ? AgentsTMP[index].WaterMaxCapacity(world) : Agents[index].WaterMaxCapacity(world))
		: 0f;

	public float GetWaterCapacity(AgroWorld world, Vector3i index) => GetWaterCapacity(world, Index(index));
