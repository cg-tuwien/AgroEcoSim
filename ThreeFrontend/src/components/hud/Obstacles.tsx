...
export class Obstacles extends Component
{
    mapItem(x: Obstacle, i: number) {
        const common = <>
            ...
            <input min={0} max={10000} step={0.1} type="number" name={`obstacleuh-${i}`} value={x.height} onChange={e => x.height.value = parseFloat(e.currentTarget.value)} />
            <label for={`obstacleut-${i}`}>thick:</label>
            <input min={0} max={Math.max(appstate.fieldSizeX.value, appstate.fieldSizeZ.value) * 2} step={0.1} type="number" name={`obstacleut-${i}`} value={x.thickness} onChange={e => x.thickness.value = parseFloat(e.currentTarget.value)} />
            </>;
        switch(x.type.value)
        {
            case "wall": return <>
                <label for={`obstaclewl-${i}`}>length:</label>
                <input min={0} max={Math.max(appstate.fieldSizeX.value, appstate.fieldSizeZ.value) * 2} step={0.1} type="number" name={`obstaclewl-${i}`} value={x.wallLength_UmbrellaRadius} onChange={e => x.wallLength_UmbrellaRadius.value = parseFloat(e.currentTarget.value)} />
                {common}
            ...
            case "umbrella": return <>
                ...
                <input min={0} max={Math.max(appstate.fieldSizeX.value, appstate.fieldSizeZ.value) * 2} step={0.1}type="number" name={`obstacleur-${i}`} value={x.wallLength_UmbrellaRadius} onChange={e => x.wallLength_UmbrellaRadius.value = parseFloat(e.currentTarget.value)} />
                {common}
                </>;
            default: return (<></>);
        }
    ...
    wrapItem(x:Obstacle, i: number) {
        return (<>
                ...
                <input max={-appstate.fieldSizeD.value} min={appstate.fieldSizeD.value} step={0.1} type="number" name={`obstaclepy-${i}`} value={x.py} onChange={e => x.py.value = parseFloat(e.currentTarget.value)}  />
